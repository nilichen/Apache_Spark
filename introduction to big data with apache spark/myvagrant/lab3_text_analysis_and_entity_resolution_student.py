...
def tokenize(string):
    ...
    """
    return [x for x in re.split(r'\W+', string.lower()) if x != '' and x not in stopwords]
...
def tf(tokens):
    ...
    return {k: v/float(len(tokens)) for k, v in tokenMap.iteritems()}
...
def tfidf(tokens, idfs):
    ...
    """
    tfs = tf(tokens)
    tfIdfDict = {k: v * idfs[k] for k, v in tfs.iteritems() }
    return tfIdfDict
...
avgSimDups = trueDupsRDD.map(lambda (x, (y, z)): y).sum()/float(trueDupsCount)

nonDupsRDD = (sims
              .subtractByKey(goldStandard))
avgSimNon = nonDupsRDD.map(lambda (x, y): y).sum()/float(nonDupsRDD.count())
...
def fastCosineSimilarity(record):
    ...
    s = sum([amazonWeightsBroadcast.value[amazonRec][token] * googleWeightsBroadcast.value[googleRec][token] 
             for token in tokens]) 
    value = s / (float(amazonNormsBroadcast.value[amazonRec]) * float(googleNormsBroadcast.value[googleRec]))

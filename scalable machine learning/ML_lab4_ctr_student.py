...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    """
    return SparseVector(numOHEFeats, [(OHEDict[feature], 1) for feature in rawFeats])
...
def parsePoint(point):
    ...
    """
    return [(count, value) for count, value in enumerate(point.split(",")[1:])]
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    Returns:
        SparseVector: A SparseVector of length numOHEFeats with indicies equal to the unique
            ...
            with values equal to 1.0.
    """
    return SparseVector(numOHEFeats, [(OHEDict[feature], 1.0) for feature in rawFeats if feature in OHEDict])
...
def computeLogLoss(p, y):
    ...
    epsilon = 10e-12
    if y == 1:
        return -log(epsilon) if p == 0 else -log(p)
    if y == 0:
        return -log(epsilon) if p == 1 else -log(1-p)
...
def computeSparsity(data, d, n):
    ...
    """
    return data.map(lambda p: len(p.features.indices) / float(d)).mean()
